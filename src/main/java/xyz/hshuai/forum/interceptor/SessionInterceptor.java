...
import xyz.hshuai.forum.mapper.UserMapper;
import xyz.hshuai.forum.service.AdService;
...
public class SessionInterceptor implements HandlerInterceptor {
   ...
   /* @Value("${github.redirect.uri}")
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        String referer = request.getHeader("referer");//告知服务器请求的原始资源的URI，其用于所有类型的请求，
      ...
      //用来说明请求从哪里发起的，包括，且仅仅包括协议和域名。post请求才有，这个参数一般只
      存在于CORS跨域请求中，可以看到response有对应的header：Access-Control-Allow-Origin。
               ...
               /* String userId;
                ...
                if (user == null) {
                    throw new RuntimeException("用户不存在，请重新登录");
                            ...
                            // 验证 token
              /*  JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(user.getPassword())).withIssuer("hshuaiUser").build();
                try {
