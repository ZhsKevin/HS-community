...
layui.define(['layer', 'laytpl', 'form', 'element', 'upload', 'util'], function(exports){
    ...
    if(device.ie && device.ie < 8){
        layer.alert('如果您非得使用 IE 浏览器访问H&S社区，那么请使用 IE8+');
    }
    ...
    var fly = {
        ...
        ,talkEditor: function(options){
            var html = ['<div class="layui-unselect fly-edit">'
                ,'<span type="face" title="插入表情"><i class="iconfont icon-yxj-expression" style="top: 1px;"></i></span>'
                ,'<span id="insert_picture_span" type="picture" title="插入图片：img[src]"><i class="iconfont icon-tupian"></i></span>'
                ,'<span type="href" title="超链接格式：a(href)[text]"><i class="iconfont icon-lianjie"></i></span>'
                ,'<span type="code" title="插入代码或引用"><i class="iconfont icon-emwdaima" style="top: 1px;"></i></span>'
                //,'<span type="hr" title="插入水平线">hr</span>'
                ,'<span id="insert_video_span" type="video" title="插入视频"><i class="layui-icon">&#xe6ed;</i></span>'
                ,'<span type="yulan" title="预览"><i class="iconfont icon-yulan1"></i></span>'
                ,'</div>'].join('');
            ...
            var log = {}, mod = {
                ...
                ,picture: function(editor){ //插入图片
                    layer.open({
                        ...
                        ,skin: 'layui-layer-border'
                        ,content: ['<ul class="layui-form layui-form-pane" style="margin: 20px;">'
                            ,'<blockquote class="layui-elem-quote">支持多图上传、拖拽上传</blockquote>'
                            ,'<li class="layui-form-item">'
                            //,'<label class="layui-form-label">URL</label>'
                            //,'<div class="layui-input-inline">'
                            //,'<input required name="image" placeholder="支持直接粘贴远程图片地址" value="" class="layui-input">'
                            ...
                            ,'<button type="button" class="layui-btn layui-btn-primary" style="margin-right: 5px" id="btn_multiple_upload_img"><i class="layui-icon">&#xe67c;</i>上传图片</button>'
                            ,'<button type="button" class="layui-btn layui-btn-primary" id="btn_image_clear" onclick="clearMultipleImgs();"><i class="layui-icon">&#x1006;\n</i>清除图片</button>'
                            ,'</div>'
                            ,'</li>'
                            ,'<li class="layui-form-item" style="text-align: center;">'
                            ,'<button type="button" lay-submit lay-filter="uploadImages" class="layui-btn">确认</button>'
                        ...
                        ,success: function(layero, index){
                            //var image =  layero.find('input[name="image"]');
                            if($('.upload_image_url').val()!=null&&$('.upload_image_url').val()!=''){
                                multiple_images = $('.upload_image_url').val().split(",");
                                for(j = 0,len=multiple_images.length; j < len; j++) {
                                    $('#upload_image_list').append('<img style="height: 66px;margin-left: 3% ;max-width: 30%" src="'+ multiple_images[j] +'" title="单击删除" onclick="delMultipleImgs(this)" class="layui-upload-img">');
                                }
                            }
                          // alert(""+$(".upload_image_url").val())
                            //执行上传实例
                            upload.render({
                               ...
                               /* ,before: function(obj){
                                    ...
                                    obj.preview(function(index, file, result){
                                        $('#upload_image_list').append('<img style="height: 66px;margin-left: 3% ;max-width: 30%" src="'+ result +'" title="单击删除" onclick="delMultipleImgs(this)" class="layui-upload-img">');
                                    });
                                }*/
                                ,done: function(res){
                                    // console.log("res:"+res)
                                    ...
                                    if(res.code == 0){
                                        ...
                                        $('#upload_image_list').append('<img style="height: 66px;margin-left: 3% ;max-width: 30%" src="'+ res.data +'" title="单击删除" onclick="delMultipleImgs(this)" class="layui-upload-img">');
                                        //console.log("upload_image_url:"+upload_image_url);
                                    } else {
                                        layer.msg(res.msg, {icon: 5});
                                        //layer.msg(res.message, {icon: 5});
                                    }
                            ...
                            form.on('submit(uploadImages)', function(data){
                                ...
                                if($('.upload_image_url').val()!=null&&$('.upload_image_url').val()!=''){
                                    ...
                                    if(multiple_images.length>0){
                                        layer.tips('已成功插入'+multiple_images.length+'张图片，您可点击再次编辑', $('#insert_picture_span'), {
                                            tips: 3
                                            ,tipsMore: true
                                            ,fixed: true
                                        });
